...
function quotaSummary(account: AccountView) {
  if (!account.snapshot) return '尚未获取'
  const credits = account.snapshot.credits
  const field = ['remainingCredits', 'remaining', 'balance', 'availableCredits', 'available'].find(key => typeof credits[key] === 'number')
  return field ? formatNumber(credits[field]) : '查看额度详情'
}
</script>
<template>
  ...
  <section class="table-panel">
    ...
    <div v-else class="table-scroll"><table class="data-table"><thead><tr><th class="checkbox-cell"><input type="checkbox" aria-label="选择当前页全部账号" :checked="allSelected" :indeterminate="selected.length > 0 && !allSelected" @change="toggleAll"></th><th>账号</th><th>状态</th><th>套餐 / 额度</th><th>并发</th><th>最近同步</th><th class="align-right">操作</th></tr></thead><tbody><tr v-for="account in data.items" :key="account.id" :class="{ selected: selected.includes(account.id), 'muted-row': !account.enabled }"><td class="checkbox-cell"><input v-model="selected" type="checkbox" :value="account.id" :aria-label="'选择 ' + (account.label || account.email || account.id)"></td><td><NuxtLink :to="'/accounts/' + account.id" class="account-name">{{ account.label || account.email || '未命名账号' }}</NuxtLink><div class="cell-secondary">{{ account.email || '未获取邮箱' }}<span v-if="account.groupName" class="group-label">{{ account.groupName }}</span></div></td><td><StatusBadge :status="account.status" /><div class="cell-secondary">{{ account.enabled ? '已启用' : '已停用' }}<span v-if="!account.hasApiKey"> · 暂无 API Key</span></div></td><td><span>{{ account.snapshot?.subscription.planId || '套餐未知' }}</span><div class="cell-secondary mono">{{ quotaSummary(account) }}</div></td><td class="mono">{{ account.inFlight }} <span class="muted">/ {{ account.maxConcurrency }}</span></td><td><span class="date-text">{{ formatDate(account.lastSyncAt) }}</span><div v-if="account.syncError" class="cell-error" :title="account.syncError">{{ account.syncError }}</div></td><td class="align-right"><NuxtLink :to="'/accounts/' + account.id" class="button small">详情<UIcon name="i-ph-arrow-right-bold" /></NuxtLink></td></tr></tbody></table></div>
